...
(async()=>{
 ...
 check('single-page filter disables paging',()=>{e("wsFilter('category','叙事')");assert.ok(q('.ws-gallery-pages').textContent.includes('1 / 1'));assert.ok([...q('.ws-gallery-pages').querySelectorAll('button')].every(b=>b.disabled));});
