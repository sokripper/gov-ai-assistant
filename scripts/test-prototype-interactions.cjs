...
(async()=>{
  ...
  await test('file tool results are isolated and restored per tool',async t=>{tool(t,'read');t.eval('ftPick()');await t.eval("ftRun('read')");t.eval("ftSwitch(1,'read')");tool(t,'ocr');assert.equal(t.eval('ftLoaded'),false);assert.equal(t.eval('ftDone'),false);tool(t,'read');assert.equal(t.eval('ftDone'),true);assert.equal(t.eval('ftTab'),1);assert.ok(!t.q('#ftOut .empty'));});
  await test('removing file clears result and prevents reopening stale tab',async t=>{tool(t,'read');t.eval('ftPick()');await t.eval("ftRun('read')");t.eval("ftRemove();ftSwitch(1,'read')");assert.equal(t.eval('ftDone'),false);assert.ok(t.q('#ftOut .empty'));});
  await test('removing file during processing cancels result',async t=>{tool(t,'read');t.eval('ftPick()');const pending=t.eval("ftRun('read')");t.eval('ftRemove()');await pending;assert.equal(t.eval('ftDone'),false);assert.ok(t.q('#ftOut .empty'));assert.equal(t.q('#ftRun').disabled,false);});
  ...
  await test('file parameters preserved after replacing file and revisiting',t=>{tool(t,'read');const select=t.q('.wb-l select');select.selectedIndex=1;const val=select.value;t.eval('ftPick()');assert.equal(t.q('.wb-l select').value,val);t.eval("go('home')");tool(t,'read');assert.equal(t.q('.wb-l select').value,val);});
