...
2026-09-18 最新库同步：01–20 改为 preserved 内联嵌套结构，21–40 保持原结构。导入脚本补齐缺失色板；适配器自动提取非固定封面的流式标题、正文容器、图片框。69项检查已改为新01–20结构、占位文本清理和原稿完整性，不继续要求旧14目录/15并列布局存在。分类分页检查按最新清单计算。
