...
(() => {
  ...
  async function loadSchool(confirmed=false,enter=false){
    ...
    ui.source='';render();if(enter)await wxGenerateArticle();else toast('已载入文章和图片','ok');
  }
  ...
  function fitThumbnails(){
    ...
    const fit=thumb=>{const content=thumb.firstElementChild;if(!content||!thumb.isConnected)return;const width=content.offsetWidth,height=content.scrollHeight;if(!width||!height||!thumb.clientWidth||!thumb.clientHeight)return;const scale=Math.min((thumb.clientWidth-16)/width,(thumb.clientHeight-16)/height);content.style.transform=`scale(${scale})`;content.style.left=((thumb.clientWidth-width*scale)/2)+'px';content.style.top='8px';thumb.dataset.fitted='true';};
    if(window.ResizeObserver)thumbObserver=new ResizeObserver(entries=>entries.forEach(({target})=>fit(target.classList.contains('ws-local-thumb')?target:target.parentElement)));
    document.querySelectorAll('.ws-local-thumb').forEach(thumb=>{fit(thumb);thumbObserver?.observe(thumb);thumbObserver?.observe(thumb.firstElementChild);thumb.querySelectorAll('img').forEach(img=>{img.addEventListener('load',()=>fit(thumb),{once:true});img.addEventListener('error',()=>fit(thumb),{once:true});});});
  }
  ...
  function inspector(){
    ...
    if(ui.panel==='versions'){
      ...
      body=`<div class="ws-review"><h3>本版已保存</h3><p>还想调整？可以继续修改，不需要重新上传或从头生成。</p><button class="btn pri" onclick="wsSelect('overall')">继续修改</button></div><p class="ws-help">${story?'修改后的草图已保存，画面待更新。':(headResult(headState().selected)?'文章与头图已保存，请核对内容后再发布。':'排版与头图方案已保存，最终画面待确认。')}</p><button class="btn" onclick="${story?"toast('长图导出将在画面生成后开放','warn')":"wxCopyLayout()"}">${story?'导出长图':'复制文章文字'}</button><p class="ws-help"></p>`;
  ...
  function finish(){
    ...
    saveVersion(wxState.outputMode==='story'?(wxCanShowStory()?'长图确认版':'已确认草图（待生成）'):(headResult(headState().selected)?'图文排版确认版':'排版方案（头图待生成）'),false);ui.done=true;ui.panel='edit';render();
  }
  ...
  async function storyExample(confirmed=false){
    if(!confirmed&&(wxState.approvedText||wxState.images.length))return openDlg('<div class="pnl-h"><h3>查看已有长图成稿？</h3></div><div class="pnl-b">将替换当前素材与会话内修改。原始文件不会删除。</div><div class="pnl-f"><button class="btn" onclick="closeAll()">取消</button><button class="btn pri" onclick="closeAll();wsStoryExample(true)">确认查看</button></div>',460);
  ...
  wxGenerateArticle=async function(refresh=false){
    ...
    wxState.step=2;init();if(fresh){if(isSchool())seedSchool();ui.versions=[];ui.next=1;saveVersion(isSchool()?'校企图文初稿':'排版初稿',false);}render();toast(isSchool()?'已打开图文成稿，可继续修改':'已载入原稿，请选择排版','ok');
  };
